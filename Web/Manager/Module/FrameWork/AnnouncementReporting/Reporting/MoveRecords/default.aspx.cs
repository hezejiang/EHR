...
namespace FrameWork.web.Module.FrameWork.AnnouncementReporting.Reporting.MoveRecords
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                ViewState["SearchTerms"] = string.Format("R_Type={0} and", 1);
            }
            else
            {
                ViewState["SearchTerms"] = string.Format("R_Type={0} and R_GroupID in ({1}) and", 1, GroupIDs);
            }
        ...
        private string SearchTerms
        {
            get
            {
                if (ViewState["SearchTerms"] == null)
                {
                    if (UserData.GetUserDate.U_Type == 0)//如果是超级管理员
                    {
                        ViewState["SearchTerms"] = string.Format("R_Type={0}", 1);
                    }
                    else
                    {
                        ViewState["SearchTerms"] = string.Format("R_Type={0} and R_GroupID in ({1})", 1, GroupIDs);
                    }
                }
