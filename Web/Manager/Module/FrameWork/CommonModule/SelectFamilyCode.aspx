<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SelectFamilyCode.aspx.cs" Inherits="FrameWork.web.Module.FrameWork.CommonModule.SelectFamilyCode" %>
...
<body>
    ...
    <div class="mini-toolbar" style="text-align:center;line-height:30px;" borderStyle="border:0;">
          <label >家庭健康档案号：</label>
          <input id="F_FamilyCode" class="mini-textbox" runat="server" style="width:150px;"/>
    ...
    <asp:GridView ID="GridView1" runat="server" OnSorting="GridView1_Sorting" 
        ...
        <Columns>
            <asp:TemplateField HeaderText="选择">
                    <ItemTemplate>
                        <input type="radio" class="selectIetm" name="selectIetm" F_FamilyCode='<%#Eval("F_FamilyCode") %>' FamilyID='<%#Eval("FamilyID") %>' />
                    </ItemTemplate>
             </asp:TemplateField>
            <asp:BoundField SortExpression="F_FamilyCode" HeaderText="家庭健康档案号" DataField="F_FamilyCode"/>
            <asp:TemplateField HeaderText="户主姓名">
                    <ItemTemplate>
...
<script type="text/javascript">
    ...
    function CloseWindow(action) {
        if (window.CloseOwnerWindow) {
            var F_FamilyCode = $(".selectIetm:checked").attr("F_FamilyCode");
            var FamilyID = $(".selectIetm:checked").attr("FamilyID");
            return window.CloseOwnerWindow(action + "||" + FamilyID + "||" + F_FamilyCode);
        }
        else
            window.close();
    }
